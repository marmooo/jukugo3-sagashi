...
function showHint(reply){reply=calcReply();showSolved(reply,true);}
...
const meiroObj=document.getElementById("meiro");resizeFontSize(meiroObj);window.addEventListener("resize",function(){resizeFontSize(meiroObj);});fetch("words.lst").then((response)=>response.text()).then((text)=>{text.trim().split("\n").forEach((line)=>{idiomsList.push(line.split(","));});idioms=idiomsList[3];generateGame();strictSolution();while(solvedPanel.firstChild){solvedPanel.removeChild(solvedPanel.firstChild);}
showAnswer();});document.getElementById("toggleDarkMode").onclick=toggleDarkMode;document.getElementById("startButton").onclick=startGame;document.getElementById("answerButton").onclick=showAnswer;document.getElementById("hintButton").onclick=showHint;document.getElementById("levelOption").addEventListener("change",function(){idioms=idiomsList[this.selectedIndex];words=wordsList[this.selectedIndex];startGame();});